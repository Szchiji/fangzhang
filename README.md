...
- [x] 时光秘匣收藏
- [x] 管理员审核面板
- [x] Mini App Leaflet 地图（含 Marker 聚合聚类）
- [x] Web API（灯笼/信用/收藏）
- [x] 匿名月影聊天会话
...
> 详细设计请参阅 [UPGRADE_PLAN.md](./UPGRADE_PLAN.md)。

### 🤖 AI 与核心匹配升级
- **pgvector 语义搜索**：在现有 PostgreSQL 中启用 `pgvector` 扩展，将灯笼描述向量化，支持自然语言（"温柔、信义区附近"）的精准 RAG 推荐。
- **pHash 网图黑名单**：对通过审核的图片计算感知哈希，防止跨用户重复提交假图。
- **多模态视频鉴真**：接受短视频投稿并逐帧鉴别真实性。

### 🗺 Mini App 交互升级
- **地图聚合聚类**（已完成）：引入 Leaflet.markercluster，灯笼密集时自动聚合为数字气泡。
- **动态路线导航**：弹出框新增"导航"按钮，跳转高德 / Google Maps。
- **骨架屏 & PWA 适配**：深色模式跟随 Telegram 主题，支持 Service Worker 离线缓存。

### 🌸 兰花信用生态与游戏化
- **等级系统**：寻花客 → 摘星人 → 月影传奇，不同等级解锁查询配额和隐藏资源。
- **匿名评价打分**：体验后通过 Inline Keyboard 提交标签式匿名反馈，反哺 AI 推荐权重。

### 🛡 群管与流量反哺（吸引群主）
- **AI 自动打假拦截**：机器人作为群管理员时，自动检测群内假图并提示。
- **月影阁主特权**：群主自动获得 VIP 身份，享有无限查询、一票否决等特权。
- **群内盲盒灯笼掉落**：定时或按消息量触发极品灯笼掉落，刺激群内活跃度。
- **月影联盟双向引流**：优质接入群组上榜官方推荐，实现流量反哺。
- **云端黑名单共享库**：跨群共享骗子/中介 Telegram ID，一处举报全网拦截。

### 📡 Telegram 生态深度集成
- **Inline Mode 分享卡片**：`@机器人 关键词` 快速将灯笼卡片分享给好友，实现裂变传播。
- **社区传奇频道推送**：高分高收藏灯笼自动图文推送到官方 Channel，自动引流。
- **月影提醒订阅**：`/subscribe 城市 类型` 订阅新灯笼上架通知。
