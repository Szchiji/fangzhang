...
# 核心修复：注册过滤器解决 TemplateAssertionError
...
def init_db():
    with get_db() as conn:
        ...
        conn.execute('''CREATE TABLE IF NOT EXISTS groups (
            ...
            list_template TEXT DEFAULT '{onlineEmoji} {地区} {name} {胸围} {价位}',
...
async def group_handler(msg: types.Message):
    ...
    if msg.text == "打卡" and user:
        with get_db() as conn:
            ...
            if exist: return await msg.reply("ℹ️ 今日已打卡")
