...
DB_PATH = os.getenv("DB_PATH", "/data/bot_v6.db")
...
async def cmd_start(msg: types.Message):
    ...
    await msg.reply(f"🚀 <b>控制中枢已就绪</b>", reply_markup=kb)
...
async def handle_msg(msg: types.Message):
    ...
    if text == g[3]: # 打卡
        ...
        await msg.reply(parse_msg(g[7] or "{名字} 已打卡", u, g[1]))
    elif text == g[9]: # 查询
...
async def api_add_task(sid: str = Form(...), gid: str = Form(...), remark: str = Form(...), content: str = Form(...), cron: int = Form(...), m_type: str = Form("text"), m_url: str = Form(None), btn: str = Form(None)):
    ...
    db_exec("INSERT INTO tasks VALUES (?,?,?,?,?,?,?,?)", (tid, gid, content, cron, remark, m_type, m_url, btn))
    return {"status": "ok"}
