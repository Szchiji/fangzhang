...
def init_db():
    with get_db() as conn:
        conn.execute('''CREATE TABLE IF NOT EXISTS groups (
            ...
            list_template TEXT DEFAULT '✅ {地区} {name} [价格:{价格}]',
            custom_fields TEXT DEFAULT '地区,价格,身材')''')
...
async def group_handler(msg: types.Message):
    ...
    if msg.text in ["今日榨汁", "今日名单"]:
        ...
        if not users: return await msg.answer("📅 今日名单为空。")
        res = f"<b>📅 今日名单 ({len(users)}人)</b>\n\n"
        for u in users:
            data = json.loads(u['data_json']); data['name'] = u['name']
            try: res += group['list_template'].format(**data) + "\n"
            except: res += f"✅ {u['name']} (占位符不匹配)\n"
...
async def cmd_start(msg: types.Message):
    ...
    kb = InlineKeyboardBuilder().button(text="🔐 进入后台", url=f"{DOMAIN}/login?sid={sid}").as_markup()
...
async def save_user(request: Request):
    ...
    fixed = ['sid', 'gid', 'user_id', 'name', 'days', 'sort']
    custom = {k: v for k, v in form.items() if k not in fixed}
